 ...
 * 初始化着色器
 */
GLuint initShader(const char *strShaderCode, unsigned int shaderType)
{
    GLuint shader = 0;
    //申请shader引用
 ...
 * 检查着色器初始化情况
 */
GLint checkInitShader(GLint pShader)
{
    //获取Shader编译情况
    GLint shaderStatu = -1;
 ...
 * 初始化渲染程序
 */
GLuint initProgram(GLuint *shaderArray, GLint size)
{
    //创建程序
 ...
 * 检查程序链接状态
 */
GLint checkLinkProgram(GLint pProgram)
{
    GLint linkProgramStatu = 0;
    glGetProgramiv(pProgram,
                   GL_LINK_STATUS, &linkProgramStatu);
